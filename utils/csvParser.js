...
const convertCsvToJsonAndImport = (csvFilePath) => {
    return new Promise((resolve, reject) => {
        ...
        stream
            .on('data', (row) => { // Event for each row in CSV
                try {
                    const movie = { // Parse row into a movie object
                        ...
                        tagline: row.tagline || '',
                        genres: row.genres ? row.genres.split(',').map((genre) => genre.trim()) : [], // Split string into array
                    ...
                    if (movies.length >= BATCH_SIZE) { // Check if batch size is reached
            ...
            .on('end', () => { // Event for end of file
                if (movies.length > 0) { // Check for remaining movies
            ...
            .on('error', (error) => { // Handle CSV read/parse errors
                stream.destroy(); // Stop stream on error
                reject(error); // Reject promise with error
            });
    });
